...
export const resourceSchema = z.object({
  ...
  resource_type: short,
  provider: short,
  release_version: z.string().max(200).default(''),
  publisher_coverage: z.string().max(1000).default(''),
  code_licence: z.string().max(300).default(''),
  model_licence: z.string().max(300).default(''),
  dataset_licence: z.string().max(300).default(''),
  training_permission: z.string().max(300).default('Unknown — review required'),
  redistribution_status: z.string().max(300).default('Unknown — review required'),
  commercial_use: z.string().max(300).default('Unknown — review required'),
  contributor_consent: z.string().max(300).default('Unknown — review required'),
  provenance_status: z.string().max(300).default('Initial assessment'),
  evidence_status: z.string().max(300).default('Documentation reviewed'),
  uncertainties: z.string().max(2000).default(''),
  status: z.enum(['Draft', 'Published']),
});
